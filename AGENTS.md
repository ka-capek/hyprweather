...
Druhá část: mlha nyní integruje prostorovou hustotu podél 16 vzorků perspektivního paprsku, s výškovým úbytkem, měkkým vlastním stínováním a protaženými cáry. Vzdálený pás oparu naznačuje hloubku bez země/moře. Používá již načtenou 3D shape texturu, žádný další fullscreen průchod ani render target, mimo mlhu se výpočet přeskočí. Denní a noční odstín se liší. Build a kontrolní render WMO 45 ve dne i v noci prošly bez shaderových chyb, snímky vizuálně zkontrolovány. Nativní vzhled světla a výkon musí potvrdit uživatel na GPU; SwiftShader měření není důkaz FPS.

Třetí část: experiment se srážkami na skle a UI je implementovaný v `js/weather-contacts.js`. Několik průsvitných kapek se zaleskne a pomalu sklouzne; jednotlivé vločky se na skle krátce zachytí. Příchozí částice narážejí na skutečné horní hrany panelů a tlačítka města, déšť se rozstříkne, sníh se rozpadne a chvíli zůstane na hraně. Jde o kreslenou optickou iluzi kapek, nikoli fyzikální refrakci obsahu UI. Rozvržení se nemění a vrstva neblokuje vstup. Renderer předává pouze změny cílového počasí, takže testovací šipky a Escape ovládají i kontaktní efekty. Při suchu nebo chybě pozadí se vyčistí a animační smyčka skončí. Limit 64 recyklovaných částic, maximálně 7 stop na skle; překreslují se malé oblasti, měření DOM jen při změně rozměrů.

Ověření celého průchodu: build a 13 testovacích souborů prošly, včetně časového běhu kontaktních částic, omezené paměti, nárazů, resize a suchého klidu. Electron ověřil živý start, šipky/Escape, dialog města, synchronizaci kontaktních efektů s WMO 65/75/0, pointer-events a resize. Kontrolní snímky hlavní aplikace s deštěm/sněhem byly prohlédnuté. Všechny položky tohoto vizuálního TODO mají nyní implementaci pro uživatelovo průběžné testování; neznamená to jeho finální vizuální schválení.
